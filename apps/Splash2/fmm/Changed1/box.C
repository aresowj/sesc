...
FreeBoxes (int my_id)
{
   ...
   for (i = 0; i < Local[my_id].Index_B_Heap; i++){
      CLEARPAUSE(b_array[i].upward_done);
      CLEARPAUSE(b_array[i].downward_done);
      CLEARPAUSE(b_array[i].construct_done);
...
InitBox (int my_id, real x_center, real y_center, real length, box *parent)
{
   ...
   PAUSEINIT(b->upward_done);
   PAUSEINIT(b->downward_done);
   PAUSEINIT(b->construct_done);
   return b;
}
