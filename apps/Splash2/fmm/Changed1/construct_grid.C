...
InsertParticlesInTree (int my_id, particle **p_list, int num_of_particles, box *root)
{
   ...
   for (i = 0; i < num_of_particles; i++) {
      ...
      MONRACES();
      while (CONDITION(dest_box->num_particles > MAX_PARTICLES_PER_BOX)) {
...
MergeLocalGrid (int my_id)
{
   ...
   myGrid=Grid;
   MONRACES();
   MLGHelper(my_id, Local[my_id].Local_Grid, myGrid, NULL);
}
	 ...
	 if (global_box == NULL) {
	    ...
	    if (global_box->type == PARENT) {
		 ...
		 if (local_box->children[i] != NULL){
		   box *gchildren;
		   IGNRACES();
		   gchildren=global_box->children[i];
		   MONRACES();
		   MLGHelper(my_id, local_box->children[i],
			     gchildren, global_box);
		 }
	       }
	    }
	    else {
		  ...
		  int numParticles;
		  IGNRACES();
		  numParticles=global_box->num_particles;
		  MONRACES();
		  InsertParticlesInTree(my_id, global_box->particles, 
					numParticles, local_box);
		  success = InsertBoxInGrid(my_id, local_box, global_parent);
	       }
	    }
	 }
...
InsertParticlesInLeaf (int my_id, particle **p_array, int length, box *b)
{
   ...
   int myNumParticles;
   IGNRACES();
   myNumParticles=b->num_particles;
   MONRACES();
  
   if ((length + myNumParticles) > MAX_PARTICLES_PER_BOX) {
      for (i = myNumParticles, j = length - 1; i < MAX_PARTICLES_PER_BOX;
	   i++, j--)
...
SetColleagues (int my_id, box *b)
{
   ...
   if (pb != NULL) {
	 ...
	 b->colleagues[b->num_colleagues++] = b->siblings[i];
      WAITPAUSE(b->construct_done);
      while (b->construct_synch == 0) {
	 /* wait */;
      }
      b->construct_synch = 0;
      CLEARPAUSE(b->construct_done);
   ...
   if (b->type == PARENT) {
	 ...
	 if (b->children[i] != NULL) {
	    ...
	    SETPAUSE(b->children[i]->construct_done);
	 }
      }
   }
}
