...
UpwardPass (int my_id, box *b)
{
   ...
   if (b->type == CHILDLESS) {
      ...
      SETPAUSE(b->upward_done);
      AULOCK(G_Memory->lock_array, b->exp_lock_index);
   }
   else {
      WAITPAUSE(b->upward_done);
   ...
   if (b->parent != NULL) {
      ...
      /*       if(b->parent->interaction_synch == b->parent->num_children)*/
      /* 	SETPAUSE(b->parent->upward_done);*/
...
DownwardPass (int my_id, box *b)
{
   ...
   if (b->parent != NULL) {
      WAITPAUSE(b->parent->downward_done);
...
ShiftMPExp (int my_id, box *cb, box *pb)
{
   ...
   if(pb->interaction_synch==0){
     for (i = 0; i < Expansion_Terms; i++) {
       pb->mp_expansion[i]=result_exp[i];
     }
   }else{
     for (i = 0; i < Expansion_Terms; i++) {
   ...
   if(pb->interaction_synch == pb->num_children)
     SETPAUSE(pb->upward_done);
...
VListInteraction (int my_id, box *source_box, box *dest_box)
{
   ...
   WAITPAUSE(source_box->upward_done);
...
XListInteraction (int my_id, box *source_box, box *dest_box)
{
   ...
   if(dest_box->x_expansion_initialized){
     for (i = 0; i < Expansion_Terms; i++) {
     ...
     for (i = 0; i < Expansion_Terms; i++) {
       dest_box->x_expansion[i].r=-result_exp[i].r;
       dest_box->x_expansion[i].i=-result_exp[i].i;
     }
     dest_box->x_expansion_initialized=1;
   }
...
ShiftLocalExp (int my_id, box *pb, box *cb)
{
   ...
   z0_pow_minus_n.i = One.i;
   ALOCK(G_Memory->lock_array, pb->exp_lock_index);
   if(pb->x_expansion_initialized){
     for (i = 0; i < Expansion_Terms; i++) {
       COMPLEX_ADD(pb->local_expansion[i], pb->local_expansion[i],
		   pb->x_expansion[i]);
       temp_exp[i]=pb->local_expansion[i];
     }
   }else{
     for (i = 0; i < Expansion_Terms; i++) {
       temp_exp[i]=pb->local_expansion[i];
   ...
   AULOCK(G_Memory->lock_array, pb->exp_lock_index);
   for (i = 0; i < Expansion_Terms; i++) {
     COMPLEX_MUL(temp_exp[i], z0_pow_n, temp_exp[i]);
     COMPLEX_MUL(z0_pow_n, z0_pow_n, z0);
   }
   for (i = 0; i < Expansion_Terms; i++) {
      result_exp[i].r = (real) 0.0;
