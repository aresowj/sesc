  ...
  unsigned process_id;
{
 ...
 retry_entry:
    ...
    while( CONDITION(global->pbar_count < n_processors) )
        {
  ...
  unsigned process_id;
{
    ...
    for( i = 0 ; i < 1000 && ! finished ; i++ )
        {
            if(    (i & 0xff) == 0
               && CONDITION( (volatile)global->pbar_count >= n_processors) )
  ...
  int level ;
{
    ...
    if( CONDITION(taskq_too_long(&global->task_queue[ taskqueue_id[process_id] ])) )
        {
  ...
  unsigned process_id;
{
    ...
    /* Check existing parallelism */
    if( CONDITION(  (total_undefs < N_visibility_per_task)
       || taskq_too_long(&global->task_queue[ taskqueue_id[process_id] ]) ) )
        {
            /* Task size is small, or the queue is too long.
               Solve it immediately. */
