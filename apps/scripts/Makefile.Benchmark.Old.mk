...
SIMDIR  := $(HOME)/sim/
#XCCBDIR := /local/milos/gnu/install2/
XCCBDIR := $(HOME)/sim/gnu/install/
...
ifeq ($(SYSTEM),Linux)
  ExeTypes := Sesc SescTLS SescTLSa
  ifdef ExeType
    DefaultTargets := $(patsubst %,$(TARGET).%,$(ExeType))
  else
    DefaultTargets := $(patsubst %,$(TARGET).%,$(ExeTypes))
  endif
  ifneq ($(filter $(ExeType),Sesc SescTLS SescTLSa),)
    M4FLAGS += $(M4FLAGS.XCC) $(SIMDIR)/sesc/src/libapp/c.m4.sesc
    GFLAGS  += -O3
    GFLAGS  += -fno-delayed-branch -fno-omit-frame-pointer 
    GFLAGS  += -fno-exceptions -fno-strict-aliasing -frename-registers
    GFLAGS  += -finline-functions -funroll-loops 
    GFLAGS  += -fgcse -fgcse-lm -fgcse-sm
    GFLAGS  += -static
    GFLAGS  += -Wa,-non_shared
#    GFLAGS  += -Wa,-non_shared
    GFLAGS  += -mabi=32  -march=mips1
    GFLAGS  += -msplit-addresses
    GFLAGS  += -mno-abicalls
#    GFLAGS  += -mno-abicalls
    GFLAGS  += -fno-PIC
#    GFLAGS  += -fno-PIC
    ifneq ($(filter $(ExeType),SescTLS SescTLSa),)
      M4FLAGS += -DTLS
      GFLAGS  += -DTLS
    endif
    ifeq ($(ExeType),SescTLSa)
      GFLAGS  += -DASPECT
    endif
    GCFLAGS += $(GFLAGS)
    GCFLAGS += -I$(SIMDIR)/sesc/src/libapp/
    LFLAGS  += $(GFLAGS) $(LFLAGS.Linux)
    LFLAGS  += -Wl,--script=$(XCCBDIR)/mipseb-linux/lib/ldscripts/mint.x
    ifneq ($(filter $(ExeType),SescTLS SescTLSa),)
      LFLAGS  += -L$(SIMDIR)/runtm/obj/mipseb_obj -lapp
    else
      LFLAGS  += -L$(SIMDIR)/runcpu/obj/mipseb_obj -lapp
    endif
  endif
  ifdef ExeType
    ...
    F90FLAGS += $(GFLAGS)
    M4   := m4
    CC   := $(XCCBDIR)/bin/mipseb-linux-gcc
    CPPC := $(XCCBDIR)/bin/mipseb-linux-g++
    FC   := mipseb-linux-g77
#    FC   := $(XCCBDIR)/bin/mipseb-linux-gfortran
    F90C := $(XCCBDIR)/bin/mipseb-linux-gfortran
  endif
endif
